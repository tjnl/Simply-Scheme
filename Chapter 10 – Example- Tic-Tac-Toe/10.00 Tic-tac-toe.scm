  ...
  (let ((count-empty-squares (count (keep (lambda (x) (equal? '_ x)) position)))
    ...
    (or (and (= count-empty-squares 2)
        ...
        (and (= count-empty-squares 1)
             (= count-dud-triples 3))
        (= count-empty-squares 0)))) ;; incorporate the definition from 10.2

(define (dud-triple? triple)
  (and (= (appearances 'x triple) 1)
       (= (appearances 'o triple) 1)))

#| Tests for correctness of 10.3
(tie-game? 'oxooxxxo_) ;; should show #t
(tie-game? 'xoxxoxo_o) ;; should show #f, as we don't know whose turn it is
(tie-game? 'xo_x_xoox) ;; should show #f
(tie-game? 'x_xxxoo_o) ;; should show #f
(tie-game? 'oxo_x_xox) ;; should show #t
(tie-game? 'x__xoxoox) ;; should show #f
(tie-game? 'oxoxoxxo_) ;; should show #f, as we don't know whose turn it is
(tie-game? 'oxooxox_x) ;; should show #f, as we don't know whose turn it is
|#